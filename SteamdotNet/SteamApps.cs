...
namespace SteamdotNet
{
    internal class SteamApps : ISteamApps
    {
        public SteamAppsData.GetAppListResponse GetAppList(SteamAppsParameters.GetAppList parameters)
        {
            ...
            return parser.ParseFromURL<SteamAppsData.GetAppListResponse>(SteamdotNetFactory.CreateMethodUrl(baseURL, parameters));
        }

        public SteamAppsData.GetServersAtAddressResponse GetServersAtAddress(SteamAppsParameters.GetServersAtAdress parameters)
        {
            const string baseURL = "http://api.steampowered.com/ISteamApps/GetServersAtAddress/v1";
            BaseParser parser = ResolveParserType(parameters.BaseParameters.Format);
            return parser.ParseFromURL<SteamAppsData.GetServersAtAddressResponse>(SteamdotNetFactory.CreateMethodUrl(baseURL, parameters));
        }
        ...
        public SteamAppsData.UpToDateCheckResponse UpToDateCheck(SteamAppsParameters.UpToDateCheck parameters)
        {
            ...
            BaseParser parser = ResolveParserType(parameters.BaseParameters.Format);
            return parser.ParseFromURL<SteamAppsData.UpToDateCheckResponse>(SteamdotNetFactory.CreateMethodUrl(baseURL, parameters));
        }
